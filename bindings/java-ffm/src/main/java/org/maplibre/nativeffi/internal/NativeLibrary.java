...
public final class NativeLibrary {
    ...
    public static void load() {
        ...
        synchronized (LOCK) {
            ...
            var configuredPath = configuredPath(
                    LIBRARY_PATH_PROPERTY, System.getProperty(LIBRARY_PATH_PROPERTY));
    ...
    private static void loadPath(Path libraryPath, String source) {
        ...
        if (!Files.isRegularFile(absolutePath)) {
            throw new UnsatisfiedLinkError(
                    "Native library from " + source + " is not a regular file: " + absolutePath);
        }
    ...
    private static UnsatisfiedLinkError loadFailure(UnsatisfiedLinkError cause) {
        var error = new UnsatisfiedLinkError("Unable to load native library " + LIBRARY_NAME
                + ". Set " + LIBRARY_PATH_PROPERTY + " or " + LIBRARY_PATH_ENV
                + " to an exact library file path, or make the library available on java.library.path.");
