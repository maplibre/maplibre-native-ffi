...
public sealed unsafe class QueryStructTests
{
    ...
    {
        using var point = NativeRenderedQueryGeometry.From(new RenderedQueryGeometry.Point(new ScreenPoint(1, 2)));
        Assert.Equal((uint)mln_rendered_query_geometry_type.MLN_RENDERED_QUERY_GEOMETRY_TYPE_POINT, point.Value.type);
        Assert.Equal(1, point.Value.data.point.x);
        Assert.Equal(2, point.Value.data.point.y);

        using var box = NativeRenderedQueryGeometry.From(new RenderedQueryGeometry.Box(new ScreenBox(new ScreenPoint(3, 4), new ScreenPoint(5, 6))));
        Assert.Equal((uint)mln_rendered_query_geometry_type.MLN_RENDERED_QUERY_GEOMETRY_TYPE_BOX, box.Value.type);
        Assert.Equal(3, box.Value.data.box.min.x);
        Assert.Equal(6, box.Value.data.box.max.y);

        using var line = NativeRenderedQueryGeometry.From(new RenderedQueryGeometry.LineString([
            new ScreenPoint(7, 8),
            new ScreenPoint(9, 10),
        ]));
        Assert.Equal((uint)mln_rendered_query_geometry_type.MLN_RENDERED_QUERY_GEOMETRY_TYPE_LINE_STRING, line.Value.type);
    ...
    {
        using var rendered = NativeRenderedFeatureQueryOptions.From(new RenderedFeatureQueryOptions
        {
            LayerIds = ["roads", "labels"],
            Filter = new JsonValue.Bool(true),
        });
        Assert.Equal((uint)mln_rendered_feature_query_option_field.MLN_RENDERED_FEATURE_QUERY_OPTION_LAYER_IDS, rendered.Value.fields);
        Assert.Equal(2u, rendered.Value.layer_id_count);
        Assert.Equal("roads", RuntimeStructs.CopyUtf8(rendered.Value.layer_ids[0].data, rendered.Value.layer_ids[0].size));
        Assert.Equal((uint)mln_json_value_type.MLN_JSON_VALUE_TYPE_BOOL, rendered.Value.filter->type);

        using var source = NativeSourceFeatureQueryOptions.From(new SourceFeatureQueryOptions
        {
            SourceLayerIds = ["landuse"],
            Filter = new JsonValue.String("visible"),
        });
        Assert.Equal((uint)mln_source_feature_query_option_field.MLN_SOURCE_FEATURE_QUERY_OPTION_SOURCE_LAYER_IDS, source.Value.fields);
        Assert.Equal(1u, source.Value.source_layer_id_count);
        Assert.Equal("landuse", RuntimeStructs.CopyUtf8(source.Value.source_layer_ids[0].data, source.Value.source_layer_ids[0].size));
        Assert.Equal((uint)mln_json_value_type.MLN_JSON_VALUE_TYPE_STRING, source.Value.filter->type);
    }
    ...
    public void QueriedFeatureCopiesNestedFeatureAndOptionalFields()
    {
        ...
        var geometry = new mln_geometry
        {
            ...
            data = { line_string = new mln_coordinate_span { coordinates = coordinates, coordinate_count = 2 } },
        };
        ...
        properties[0] = new mln_json_member { key = propertyKey.Value, value = propertyValue.Pointer };
        ...
        using var state = NativeJsonValue.From(new JsonValue.Object([new JsonMember("hover", new JsonValue.Bool(true))]));
        var queried = QueryStructs.ReadQueriedFeature(new mln_queried_feature
        {
            size = (uint)sizeof(mln_queried_feature),
            fields = (uint)(mln_queried_feature_field.MLN_QUERIED_FEATURE_SOURCE_ID |
        ...
        Assert.Equal(new JsonMember("name", new JsonValue.String("park")), queried.Feature.Properties[0]);
