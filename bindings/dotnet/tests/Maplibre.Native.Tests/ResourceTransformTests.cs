...
public sealed class ResourceTransformTests
{
  ...
  public void ResourceTransformCopiesRequestWhenKeepingOriginalUrl()
  {
    ...
    Assert.Equal(
        ...
        state.TransformForTest(ResourceKind.Tile, "https://example.test/tile", out var replacementUrl));
  ...
  public void ResourceTransformEmbeddedNulReplacementMapsToInvalidArgument()
  {
    ...
    Assert.Equal(
        ...
        state.TransformForTest(ResourceKind.Style, "https://example.test/style.json", out var replacementUrl));
    Assert.Null(replacementUrl);
  }
  ...
  public unsafe void ResourceTransformResponseHelperRequiresNativeCallbackContext()
  {
    ...
    using var replacementUrl = NativeUtf8String.FromNullableString("https://example.test/style.json", "replacementUrl");

    Assert.Equal(
        mln_status.MLN_STATUS_INVALID_STATE,
        NativeMethods.mln_resource_transform_response_set_url(&response, replacementUrl.Pointer, replacementUrl.ByteLength));
