...
public sealed unsafe class OfflineStructTests
{
  ...
  {
    using var tile = NativeOfflineRegionDefinition.From(new OfflineRegionDefinition.TilePyramid(
        "maplibre://style",
    ...
    Assert.Equal((uint)mln_offline_region_definition_type.MLN_OFFLINE_REGION_DEFINITION_TILE_PYRAMID, tile.Value.type);
    Assert.Equal("maplibre://style", Marshal.PtrToStringUTF8((nint)tile.Value.data.tile_pyramid.style_url));
    ...
    using var geometry = NativeOfflineRegionDefinition.From(new OfflineRegionDefinition.GeometryRegion(
        "maplibre://geometry",
    ...
    Assert.Equal((uint)mln_offline_region_definition_type.MLN_OFFLINE_REGION_DEFINITION_GEOMETRY, geometry.Value.type);
    Assert.Equal("maplibre://geometry", Marshal.PtrToStringUTF8((nint)geometry.Value.data.geometry.style_url));
    Assert.Equal((uint)mln_geometry_type.MLN_GEOMETRY_TYPE_POINT, geometry.Value.data.geometry.geometry->type);
  ...
  public void OfflineRegionInfoCopiesDefinitionAndMetadata()
  {
    ...
    var info = OfflineStructs.ReadInfo(new mln_offline_region_info
    {
        ...
        data = { tile_pyramid = new mln_offline_tile_pyramid_region_definition
                {
                    ...
                    bounds = MapStructs.ToNative(new LatLngBounds(new LatLng(1, 2), new LatLng(3, 4))),
                    min_zoom = 5,
                    max_zoom = 6,
                    pixel_ratio = 2,
  ...
  public void OfflineRegionStatusCopiesNativeFields()
  {
    ...
    {
      download_state = (uint)mln_offline_region_download_state.MLN_OFFLINE_REGION_DOWNLOAD_ACTIVE,
