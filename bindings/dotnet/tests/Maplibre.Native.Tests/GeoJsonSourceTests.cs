...
public sealed unsafe class GeoJsonSourceTests
{
    ...
    public void GeoJsonMaterializesFeatureCollectionWithProperties()
    {
        ...
        Assert.Equal((uint)mln_geojson_type.MLN_GEOJSON_TYPE_FEATURE_COLLECTION, geoJson.Pointer->type);
        ...
        Assert.Equal((uint)mln_feature_identifier_type.MLN_FEATURE_IDENTIFIER_TYPE_STRING, nativeFeature.identifier_type);
    }
    ...
    public void GeoJsonSourceDataAdaptsThroughNativeMap()
    {
        ...
        map.SetGeoJsonSourceData("geo-data", new GeoJson.GeometryValue(new Geometry.Point(new LatLng(1, 2))));
