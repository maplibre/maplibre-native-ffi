...
internal sealed unsafe class CustomGeometrySourceState : IDisposable
{
    ...
    // The C API invokes this once after MapLibre stops referencing this state, so the stubs go
    // with the source whether it was removed, dropped by a style load, or retired with the map.
    [UnmanagedCallersOnly(CallConvs = [typeof(System.Runtime.CompilerServices.CallConvCdecl)])]
