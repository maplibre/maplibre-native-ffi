...
public static unsafe class Maplibre
{
    ...
    public static global::Maplibre.NativeFfi.NetworkStatus NetworkStatus()
    {
        NativeLibraryLoader.EnsureLoaded();
        uint status = 0;
        NativeStatus.Check(NativeMethods.mln_network_status_get(&status));
        return global::Maplibre.NativeFfi.NetworkStatus.FromRaw(status);
    }

    /// <summary>Sets MapLibre Native's process-global network status.</summary>
    public static void SetNetworkStatus(global::Maplibre.NativeFfi.NetworkStatus status)
    {
