...
public sealed unsafe class ResourceRequestHandle : IDisposable
{
    ...
    /// worker thread when a committed map or runtime command discards it, and a
    /// MapLibre worker thread otherwise, never a host thread. It runs only for
    /// a request the provider has not completed, and it may complete or close
    /// this handle. When the request is already cancelled, the callback runs on
    /// the calling thread before this method returns, as part of this call: a
    /// concurrent <see cref="Close" /> on another thread does not wait for it. A
    /// request accepts one registration, and a second registration throws
