...
public sealed class VulkanContextDescriptor { public NativePointer Instance { get; set; } public NativePointer PhysicalDevice { get; set; } public NativePointer Device { get; set; } public NativePointer Queue { get; set; } public uint GraphicsQueueFamilyIndex { get; set; } public NativePointer GetInstanceProcAddr { get; set; } public NativePointer GetDeviceProcAddr { get; set; } }
...
public sealed class OpenGLOwnedTextureFrame
{
    ...
    internal OpenGLOwnedTextureFrame(FrameScope scope, ulong generation, uint width, uint height, double scaleFactor, ulong frameId, uint texture, uint target, uint internalFormat, uint format, uint type)
    {
        this.scope = scope;
        this.generation = generation;
        this.width = width;
    ...
    public double ScaleFactor { get { scope.EnsureActive(); return scaleFactor; } }
    public ulong FrameId { get { scope.EnsureActive(); return frameId; } }
    public uint Texture { get { scope.EnsureActive(); return texture; } }
    public uint Target { get { scope.EnsureActive(); return target; } }
    public uint InternalFormat { get { scope.EnsureActive(); return internalFormat; } }
    public uint Format { get { scope.EnsureActive(); return format; } }
    public uint Type { get { scope.EnsureActive(); return type; } }
}
