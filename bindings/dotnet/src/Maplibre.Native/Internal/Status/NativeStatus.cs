  ...
  private static MaplibreException CreateException(int rawStatus, string diagnostic)
  {
    ...
    return status switch
    {
      MaplibreStatus.InvalidArgument =>
          new InvalidArgumentException(status, rawStatus, diagnostic),
      MaplibreStatus.InvalidState =>
          new InvalidStateException(status, rawStatus, diagnostic),
      MaplibreStatus.WrongThread =>
          new WrongThreadException(status, rawStatus, diagnostic),
      MaplibreStatus.Unsupported =>
          new UnsupportedFeatureException(status, rawStatus, diagnostic),
      MaplibreStatus.NativeError =>
          new NativeErrorException(status, rawStatus, diagnostic),
      _ => new MaplibreException(status, rawStatus, diagnostic),
    };
  }

  private static MaplibreStatus StatusFromRaw(int rawStatus) => rawStatus switch
  {
    0 => MaplibreStatus.Ok,
