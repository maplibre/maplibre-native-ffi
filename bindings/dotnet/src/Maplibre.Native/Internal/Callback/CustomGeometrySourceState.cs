...
internal sealed unsafe class CustomGeometrySourceState : IDisposable
{
    ...
    internal mln_custom_geometry_source_options Descriptor
    {
        get
        {
            ...
            {
                descriptor.fields |= (uint)mln_custom_geometry_source_option_field.MLN_CUSTOM_GEOMETRY_SOURCE_OPTION_MIN_ZOOM;
                descriptor.min_zoom = minimumZoom;
            }
            if (options.MaximumZoom is { } maximumZoom)
            {
                descriptor.fields |= (uint)mln_custom_geometry_source_option_field.MLN_CUSTOM_GEOMETRY_SOURCE_OPTION_MAX_ZOOM;
                descriptor.max_zoom = maximumZoom;
            }
            if (options.Tolerance is { } tolerance)
            {
                descriptor.fields |= (uint)mln_custom_geometry_source_option_field.MLN_CUSTOM_GEOMETRY_SOURCE_OPTION_TOLERANCE;
                descriptor.tolerance = tolerance;
            }
            if (options.TileSize is { } tileSize)
            {
                descriptor.fields |= (uint)mln_custom_geometry_source_option_field.MLN_CUSTOM_GEOMETRY_SOURCE_OPTION_TILE_SIZE;
            ...
            {
                descriptor.fields |= (uint)mln_custom_geometry_source_option_field.MLN_CUSTOM_GEOMETRY_SOURCE_OPTION_CLIP;
                descriptor.clip = clip ? (byte)1 : (byte)0;
            }
            if (options.Wrap is { } wrap)
            {
                descriptor.fields |= (uint)mln_custom_geometry_source_option_field.MLN_CUSTOM_GEOMETRY_SOURCE_OPTION_WRAP;
    ...
    private static CanonicalTileId FromNative(mln_canonical_tile_id tileId) => new(tileId.z, tileId.x, tileId.y);
