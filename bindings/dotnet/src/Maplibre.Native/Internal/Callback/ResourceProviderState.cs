...
internal sealed unsafe class ResourceProviderState : IDisposable
{
    ...
    private static uint OnRequest(void* userData, mln_resource_request* request, mln_resource_request_handle* requestHandle)
    {
        try
        {
    ...
    private static uint Invoke(ResourceProviderState? state, mln_resource_request* request, mln_resource_request_handle* requestHandle)
    {
