...
{
  internal static mln_edge_insets ToNative(EdgeInsets value) => new()
  {
    top = value.Top,
    left = value.Left,
    bottom = value.Bottom,
  ...
  internal static EdgeInsets FromNative(mln_edge_insets value) => new(value.top, value.left, value.bottom, value.right);

  internal static mln_screen_point ToNative(ScreenPoint value) => new()
  {
    x = value.X,
    y = value.Y,
  };
  ...
  internal static mln_lat_lng_bounds ToNative(LatLngBounds value) => new()
  {
    southwest = CoreStructs.ToNative(value.Southwest),
    northeast = CoreStructs.ToNative(value.Northeast),
  };

  internal static LatLngBounds FromNative(mln_lat_lng_bounds value) => new(CoreStructs.FromNative(value.southwest), CoreStructs.FromNative(value.northeast));

  internal static mln_vec3 ToNative(Vec3 value) => new()
  {
    x = value.X,
    y = value.Y,
    z = value.Z,
  ...
  internal static mln_quaternion ToNative(Quaternion value) => new()
  {
    x = value.X,
    y = value.Y,
    z = value.Z,
  ...
  internal static Quaternion FromNative(mln_quaternion value) => new(value.x, value.y, value.z, value.w);
  ...
  internal static mln_map_viewport_options ToNative(ViewportOptions options)
  {
    ...
    {
      native.fields |= (uint)mln_map_viewport_option_field.MLN_MAP_VIEWPORT_OPTION_NORTH_ORIENTATION;
      native.north_orientation = (uint)northOrientation;
    }
    if (options.ConstrainMode is { } constrainMode)
    {
      native.fields |= (uint)mln_map_viewport_option_field.MLN_MAP_VIEWPORT_OPTION_CONSTRAIN_MODE;
      native.constrain_mode = (uint)constrainMode;
    }
    if (options.ViewportMode is { } viewportMode)
    {
      native.fields |= (uint)mln_map_viewport_option_field.MLN_MAP_VIEWPORT_OPTION_VIEWPORT_MODE;
      native.viewport_mode = (uint)viewportMode;
    }
    if (options.FrustumOffset is { } frustumOffset)
    {
      native.fields |= (uint)mln_map_viewport_option_field.MLN_MAP_VIEWPORT_OPTION_FRUSTUM_OFFSET;
  ...
  internal static ViewportOptions ViewportOptionsFromNative(mln_map_viewport_options native)
  {
    ...
    if (Has(native.fields, mln_map_viewport_option_field.MLN_MAP_VIEWPORT_OPTION_NORTH_ORIENTATION))
    {
      options.NorthOrientation = (NorthOrientation)native.north_orientation;
    }
    if (Has(native.fields, mln_map_viewport_option_field.MLN_MAP_VIEWPORT_OPTION_CONSTRAIN_MODE))
    {
    ...
    if (Has(native.fields, mln_map_viewport_option_field.MLN_MAP_VIEWPORT_OPTION_FRUSTUM_OFFSET))
    {
  ...
  internal static mln_map_tile_options ToNative(TileOptions options)
  {
    ...
    {
      native.fields |= (uint)mln_map_tile_option_field.MLN_MAP_TILE_OPTION_PREFETCH_ZOOM_DELTA;
    ...
    {
      native.fields |= (uint)mln_map_tile_option_field.MLN_MAP_TILE_OPTION_LOD_PITCH_THRESHOLD;
