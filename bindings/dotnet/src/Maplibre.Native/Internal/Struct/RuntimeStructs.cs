...
internal static unsafe class RuntimeStructs
{
  ...
  private static RuntimeEventPayload ReadPayload(uint payloadType, void* payload, nuint payloadSize)
  {
    if (payload is null || payloadType == (uint)mln_runtime_event_payload_type.MLN_RUNTIME_EVENT_PAYLOAD_NONE)
    {
  ...
  private static RuntimeEventPayload.RenderMap ReadRenderMap(mln_runtime_event_render_map* payload) =>
      new((RenderMode)payload->mode, payload->mode);

  private static RuntimeEventPayload.StyleImageMissing ReadStyleImageMissing(mln_runtime_event_style_image_missing* payload) =>
      new(CopyUtf8(payload->image_id, payload->image_id_size));

  private static RuntimeEventPayload.TileAction ReadTileAction(mln_runtime_event_tile_action* payload) =>
      new((TileOperation)payload->operation, payload->operation, TileId(payload->tile_id), CopyUtf8(payload->source_id, payload->source_id_size));

  private static RuntimeEventPayload.OfflineRegionStatusChanged ReadOfflineRegionStatus(mln_runtime_event_offline_region_status* payload) =>
      new(payload->region_id, OfflineRegionStatus(payload->status));

  private static RuntimeEventPayload.OfflineRegionResponseError ReadOfflineRegionResponseError(mln_runtime_event_offline_region_response_error* payload) =>
      new(payload->region_id, (ResourceErrorReason)payload->reason, payload->reason);

  private static RuntimeEventPayload.OfflineRegionTileCountLimit ReadOfflineRegionTileCountLimit(mln_runtime_event_offline_region_tile_count_limit* payload) =>
      new(payload->region_id, payload->limit);

  private static RuntimeEventPayload.OfflineOperationCompleted ReadOfflineOperationCompleted(mln_runtime_event_offline_operation_completed* payload) =>
      new(payload->operation_id, (OfflineOperationKind)payload->operation_kind, payload->operation_kind, (OfflineOperationResultKind)payload->result_kind, payload->result_kind, payload->result_status, payload->found != 0);

  private static RenderingStats RenderingStats(mln_rendering_stats value) =>
      new(value.encoding_time, value.rendering_time, value.frame_count, value.draw_call_count, value.total_draw_call_count);

  private static TileId TileId(mln_tile_id value) =>
      new(value.overscaled_z, value.wrap, value.canonical_z, value.canonical_x, value.canonical_y);

  private static OfflineRegionStatus OfflineRegionStatus(mln_offline_region_status value) =>
      new((OfflineRegionDownloadState)value.download_state, value.completed_resource_count, value.completed_resource_size, value.completed_tile_count, value.required_tile_count, value.completed_tile_size, value.required_resource_count, value.required_resource_count_is_precise != 0, value.complete != 0);
