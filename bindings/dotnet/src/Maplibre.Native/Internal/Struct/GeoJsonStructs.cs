...
internal sealed unsafe class NativeGeometry : IDisposable
{
    ...
    private void Write(mln_geometry* target, Geometry geometry, int depth)
    {
        ...
        {
            throw new Error.InvalidArgumentException(Error.MaplibreStatus.InvalidArgument, null, $"Geometry exceeds maximum collection depth {Geometry.MaxCollectionDepth}.");
        }
    ...
    private mln_coordinate_span CoordinateSpan(IReadOnlyList<LatLng> coordinates)
    {
        ...
        return new mln_coordinate_span { coordinates = pointer, coordinate_count = (nuint)coordinates.Count };
    }
    ...
    private mln_multi_polygon_geometry MultiPolygon(IReadOnlyList<IReadOnlyList<IReadOnlyList<LatLng>>> polygons)
    {
        if (polygons.Count == 0)
        {
            return default;
        ...
        return new mln_multi_polygon_geometry { polygons = pointer, polygon_count = (nuint)polygons.Count };
    }
    ...
    private mln_geometry_collection Collection(IReadOnlyList<Geometry> geometries, int depth)
    {
        ...
        return new mln_geometry_collection { geometries = pointer, geometry_count = (nuint)geometries.Count };
    }
...
internal sealed unsafe class NativeFeature : IDisposable
{
    ...
    internal static NativeFeature From(Feature feature)
    {
        ...
        var native = new NativeFeature(new mln_feature { size = (uint)sizeof(mln_feature), geometry = geometry.Pointer }, geometry);
        try
        {
    ...
    private void SetIdentifier(FeatureIdentifier identifier)
    {
        ...
        switch (identifier)
        {
            case FeatureIdentifier.Null:
                current.identifier_type = (uint)mln_feature_identifier_type.MLN_FEATURE_IDENTIFIER_TYPE_NULL;
                break;
            case FeatureIdentifier.UInt unsigned:
                current.identifier_type = (uint)mln_feature_identifier_type.MLN_FEATURE_IDENTIFIER_TYPE_UINT;
                current.identifier.uint_value = unsigned.Value;
                break;
            case FeatureIdentifier.Int integer:
                current.identifier_type = (uint)mln_feature_identifier_type.MLN_FEATURE_IDENTIFIER_TYPE_INT;
                current.identifier.int_value = integer.Value;
                break;
            case FeatureIdentifier.Double number:
                current.identifier_type = (uint)mln_feature_identifier_type.MLN_FEATURE_IDENTIFIER_TYPE_DOUBLE;
                current.identifier.double_value = number.Value;
                break;
            case FeatureIdentifier.String text:
                current.identifier_type = (uint)mln_feature_identifier_type.MLN_FEATURE_IDENTIFIER_TYPE_STRING;
...
internal sealed unsafe class NativeGeoJson : IDisposable
{
    ...
    private mln_feature_collection FeatureCollection(IReadOnlyList<Feature> features)
    {
        ...
        return new mln_feature_collection { features = pointer, feature_count = (nuint)features.Count };
    }
