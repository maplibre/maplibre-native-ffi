    ...
    internal static NativeRenderedQueryGeometry From(RenderedQueryGeometry geometry)
    {
        ...
        switch (geometry)
        {
            case RenderedQueryGeometry.Point point:
                return new NativeRenderedQueryGeometry(NativeMethods.mln_rendered_query_geometry_point(MapStructs.ToNative(point.Value)), 0);
            case RenderedQueryGeometry.Box box:
                return new NativeRenderedQueryGeometry(NativeMethods.mln_rendered_query_geometry_box(new mln_screen_box
                {
            ...
            case RenderedQueryGeometry.LineString line:
                ...
                try
                {
                    return new NativeRenderedQueryGeometry(NativeMethods.mln_rendered_query_geometry_line_string(nativePoints, (nuint)count), allocation);
                }
                catch
                {
                    if (allocation != 0)
            ...
            default:
                throw new ArgumentException($"Unsupported rendered query geometry type {geometry.GetType().Name}.", nameof(geometry));
        }
    }
...
internal sealed unsafe class NativeRenderedFeatureQueryOptions : IDisposable
{
    ...
    private NativeRenderedFeatureQueryOptions(mln_rendered_feature_query_options value, NativeJsonValue? filter)
    {
        Value = value;
        this.filter = filter;
    }
    ...
    internal static NativeRenderedFeatureQueryOptions From(RenderedFeatureQueryOptions options)
    {
        ...
        try
        {
            ...
            {
                value.fields |= (uint)mln_rendered_feature_query_option_field.MLN_RENDERED_FEATURE_QUERY_OPTION_LAYER_IDS;
...
internal sealed unsafe class NativeSourceFeatureQueryOptions : IDisposable
{
    ...
    private NativeSourceFeatureQueryOptions(mln_source_feature_query_options value, NativeJsonValue? filter)
    {
        Value = value;
        this.filter = filter;
    }
    ...
    internal static NativeSourceFeatureQueryOptions From(SourceFeatureQueryOptions options)
    {
        ...
        try
        {
            ...
            {
                value.fields |= (uint)mln_source_feature_query_option_field.MLN_SOURCE_FEATURE_QUERY_OPTION_SOURCE_LAYER_IDS;
...
{
    internal static IReadOnlyList<QueriedFeature> ReadFeatureQueryResult(mln_feature_query_result* result)
    {
        if (result is null)
        {
            return [];
        ...
        try
        {
            ...
            for (nuint index = 0; index < count; index++)
            {
                ...
                NativeStatus.Check(NativeMethods.mln_feature_query_result_get(result, index, &feature));
    ...
    internal static FeatureExtensionResult ReadFeatureExtensionResult(mln_feature_extension_result* result)
    {
        if (result is null)
        {
        ...
        try
        {
            var info = new mln_feature_extension_result_info { size = (uint)sizeof(mln_feature_extension_result_info) };
            ...
            return (mln_feature_extension_result_type)info.type switch
            {
                ...
                mln_feature_extension_result_type.MLN_FEATURE_EXTENSION_RESULT_TYPE_FEATURE_COLLECTION =>
                    new FeatureExtensionResult.FeatureCollection(ReadFeatureCollection(info.data.feature_collection)),
    ...
    internal static QueriedFeature ReadQueriedFeature(mln_queried_feature value)
    {
        ...
        var sourceId = fields.HasFlag(mln_queried_feature_field.MLN_QUERIED_FEATURE_SOURCE_ID)
            ...
            : null;
        var sourceLayerId = fields.HasFlag(mln_queried_feature_field.MLN_QUERIED_FEATURE_SOURCE_LAYER_ID)
            ? RuntimeStructs.CopyUtf8(value.source_layer_id.data, value.source_layer_id.size)
            : null;
        var state = fields.HasFlag(mln_queried_feature_field.MLN_QUERIED_FEATURE_STATE) && value.state is not null
    ...
    {
        var geometry = value.geometry is null ? Geometry.Empty.Instance : ReadGeometry(*value.geometry);
        var properties = new JsonMember[checked((int)value.property_count)];
        for (var index = 0; index < properties.Length; index++)
        {
            var member = value.properties[index];
            properties[index] = new JsonMember(RuntimeStructs.CopyUtf8(member.key.data, member.key.size), ValueStructs.ReadJsonValue(member.value));
        }
        return new Feature(geometry, properties, ReadIdentifier(value));
    }

    internal static Geometry ReadGeometry(mln_geometry geometry) => (mln_geometry_type)geometry.type switch
    {
        mln_geometry_type.MLN_GEOMETRY_TYPE_EMPTY => Geometry.Empty.Instance,
        mln_geometry_type.MLN_GEOMETRY_TYPE_POINT => new Geometry.Point(CoreStructs.FromNative(geometry.data.point)),
        mln_geometry_type.MLN_GEOMETRY_TYPE_LINE_STRING => new Geometry.LineString(ReadCoordinateSpan(geometry.data.line_string)),
        mln_geometry_type.MLN_GEOMETRY_TYPE_POLYGON => new Geometry.Polygon(ReadPolygon(geometry.data.polygon)),
        mln_geometry_type.MLN_GEOMETRY_TYPE_MULTI_POINT => new Geometry.MultiPoint(ReadCoordinateSpan(geometry.data.multi_point)),
    ...
    private static IReadOnlyList<IReadOnlyList<LatLng>> ReadMultiLine(mln_multi_line_geometry multiLine)
    {
    ...
    private static IReadOnlyList<IReadOnlyList<IReadOnlyList<LatLng>>> ReadMultiPolygon(mln_multi_polygon_geometry multiPolygon)
    {
        var polygons = new IReadOnlyList<IReadOnlyList<LatLng>>[checked((int)multiPolygon.polygon_count)];
    ...
    private static IReadOnlyList<Geometry> ReadGeometryCollection(mln_geometry_collection collection)
    {
