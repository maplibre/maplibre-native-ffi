...
internal sealed unsafe class NativeOfflineRegionDefinition : IDisposable
{
    ...
    private NativeOfflineRegionDefinition(mln_offline_region_definition value, NativeUtf8String styleUrl, NativeGeometry? geometry)
    {
        Value = value;
        ...
        try
        {
            switch (definition)
            {
                case OfflineRegionDefinition.TilePyramid tile:
                    styleUrl = NativeUtf8String.FromNullableString(tile.StyleUrl, nameof(tile.StyleUrl));
                    return new NativeOfflineRegionDefinition(new mln_offline_region_definition
                    {
                        size = (uint)sizeof(mln_offline_region_definition),
                        type = (uint)mln_offline_region_definition_type.MLN_OFFLINE_REGION_DEFINITION_TILE_PYRAMID,
                        data = { tile_pyramid = new mln_offline_tile_pyramid_region_definition
                        {
                ...
                case OfflineRegionDefinition.GeometryRegion region:
                    styleUrl = NativeUtf8String.FromNullableString(region.StyleUrl, nameof(region.StyleUrl));
                    geometry = NativeGeometry.From(region.Geometry);
                    return new NativeOfflineRegionDefinition(new mln_offline_region_definition
                    {
                        size = (uint)sizeof(mln_offline_region_definition),
                        type = (uint)mln_offline_region_definition_type.MLN_OFFLINE_REGION_DEFINITION_GEOMETRY,
                        data = { geometry = new mln_offline_geometry_region_definition
                        {
                            ...
                            include_ideographs = region.IncludeIdeographs ? (byte)1 : (byte)0,
                        } },
                    }, styleUrl, geometry);
                default:
                    throw new ArgumentException($"Unsupported offline region definition type {definition.GetType().Name}.", nameof(definition));
            }
        }
        catch
        {
...
internal static unsafe class OfflineStructs
{
    ...
    private static OfflineRegionDefinition.TilePyramid ReadTilePyramid(mln_offline_tile_pyramid_region_definition value) =>
        new(
    ...
    private static OfflineRegionDefinition.GeometryRegion ReadGeometryRegion(mln_offline_geometry_region_definition value) =>
        new(
            CopyCString(value.style_url),
            value.geometry is null ? Geometry.Empty.Instance : QueryStructs.ReadGeometry(*value.geometry),
            value.min_zoom,
            value.max_zoom,
            value.pixel_ratio,
            value.include_ideographs != 0);
