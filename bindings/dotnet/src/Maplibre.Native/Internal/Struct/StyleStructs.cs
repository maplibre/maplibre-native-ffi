...
internal sealed unsafe class NativeTileSourceOptions : IDisposable
{
    ...
    private NativeTileSourceOptions(mln_style_tile_source_options value, NativeStringView? attribution)
    {
        Value = value;
    ...
    internal static NativeTileSourceOptions From(TileSourceOptions options)
    {
        ...
        try
        {
            ...
            {
                native.fields |= (uint)mln_style_tile_source_option_field.MLN_STYLE_TILE_SOURCE_OPTION_MIN_ZOOM;
                native.min_zoom = minimumZoom;
            }
            if (options.MaximumZoom is { } maximumZoom)
            {
                native.fields |= (uint)mln_style_tile_source_option_field.MLN_STYLE_TILE_SOURCE_OPTION_MAX_ZOOM;
                native.max_zoom = maximumZoom;
            }
            if (options.Attribution is { } attributionValue)
            {
                native.fields |= (uint)mln_style_tile_source_option_field.MLN_STYLE_TILE_SOURCE_OPTION_ATTRIBUTION;
            ...
            {
                native.fields |= (uint)mln_style_tile_source_option_field.MLN_STYLE_TILE_SOURCE_OPTION_SCHEME;
                native.scheme = (uint)scheme;
            }
            if (options.Bounds is { } bounds)
            {
                native.fields |= (uint)mln_style_tile_source_option_field.MLN_STYLE_TILE_SOURCE_OPTION_BOUNDS;
                native.bounds = MapStructs.ToNative(bounds);
            }
            if (options.TileSize is { } tileSize)
            {
                native.fields |= (uint)mln_style_tile_source_option_field.MLN_STYLE_TILE_SOURCE_OPTION_TILE_SIZE;
                native.tile_size = tileSize;
            }
            if (options.VectorEncoding is { } vectorEncoding)
            {
                native.fields |= (uint)mln_style_tile_source_option_field.MLN_STYLE_TILE_SOURCE_OPTION_VECTOR_ENCODING;
                native.vector_encoding = (uint)vectorEncoding;
            }
            if (options.RasterEncoding is { } rasterEncoding)
            {
                native.fields |= (uint)mln_style_tile_source_option_field.MLN_STYLE_TILE_SOURCE_OPTION_RASTER_ENCODING;
