...
const VulkanDispatch = if (build_options.supports_vulkan) struct {
    ...
    fn loadInstanceFunctions(self: *VulkanDispatch, instance: vk.VkInstance) void {
        ...
        self.get_physical_device_memory_properties = loadInstance(vk.PFN_vkGetPhysicalDeviceMemoryProperties, self.get_instance_proc_addr, instance, "vkGetPhysicalDeviceMemoryProperties") catch null;
    ...
    fn loadDeviceFunctions(self: *VulkanDispatch, device: vk.VkDevice) void {
        ...
        self.create_image = loadDevice(vk.PFN_vkCreateImage, self.get_device_proc_addr, device, "vkCreateImage") catch null;
        self.destroy_image = loadDevice(vk.PFN_vkDestroyImage, self.get_device_proc_addr, device, "vkDestroyImage") catch null;
        self.get_image_memory_requirements = loadDevice(vk.PFN_vkGetImageMemoryRequirements, self.get_device_proc_addr, device, "vkGetImageMemoryRequirements") catch null;
        self.allocate_memory = loadDevice(vk.PFN_vkAllocateMemory, self.get_device_proc_addr, device, "vkAllocateMemory") catch null;
        self.free_memory = loadDevice(vk.PFN_vkFreeMemory, self.get_device_proc_addr, device, "vkFreeMemory") catch null;
        self.bind_image_memory = loadDevice(vk.PFN_vkBindImageMemory, self.get_device_proc_addr, device, "vkBindImageMemory") catch null;
        self.create_image_view = loadDevice(vk.PFN_vkCreateImageView, self.get_device_proc_addr, device, "vkCreateImageView") catch null;
        self.destroy_image_view = loadDevice(vk.PFN_vkDestroyImageView, self.get_device_proc_addr, device, "vkDestroyImageView") catch null;
    }
} else struct {};
