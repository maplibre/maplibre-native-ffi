...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mln_runtime*`                      | `type RuntimeHandle struct` with private state and `Close() error`; retains runtime-owned callback state and a map registry for event-source side effects.                                                             |
...
| Session-owned texture frame values  | `type MetalOwnedTextureFrame` and `type VulkanOwnedTextureFrame` wrappers; close on the session owner thread, release before resize/render update/detach/session close, and expose frame-scoped native pointer access. |
