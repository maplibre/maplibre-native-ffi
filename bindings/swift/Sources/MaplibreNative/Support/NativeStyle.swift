...
enum NativeStyle {
  static func removeSource(_ map: OpaquePointer, sourceId: mln_string_view) throws -> Bool {
    try NativeMemory.withTemporary(false) { removed in
      try checkStatus(mln_map_remove_style_source(map, sourceId, removed))
    }.value
  }

  static func sourceExists(_ map: OpaquePointer, sourceId: mln_string_view) throws -> Bool {
    try NativeMemory.withTemporary(false) { exists in
      try checkStatus(mln_map_style_source_exists(map, sourceId, exists))
    }.value
  }

  static func sourceType(_ map: OpaquePointer, sourceId: mln_string_view) throws -> UInt32? {
    var type = UInt32(0)
  ...
  static func sourceInfo(_ map: OpaquePointer, sourceId: mln_string_view) throws -> NativeStyleSourceInfo? {
  ...
  static func copySourceAttribution(_ map: OpaquePointer, sourceId: mln_string_view, capacity: Int) throws -> (String?, Int) {
  ...
  static func removeImage(_ map: OpaquePointer, imageId: mln_string_view) throws -> Bool {
    try NativeMemory.withTemporary(false) { removed in
      try checkStatus(mln_map_remove_style_image(map, imageId, removed))
    }.value
  }

  static func imageExists(_ map: OpaquePointer, imageId: mln_string_view) throws -> Bool {
    try NativeMemory.withTemporary(false) { exists in
      try checkStatus(mln_map_style_image_exists(map, imageId, exists))
    }.value
  }

  static func imageInfo(_ map: OpaquePointer, imageId: mln_string_view) throws -> NativeStyleImageInfo? {
  ...
  static func copyImagePremultipliedRGBA8(_ map: OpaquePointer, imageId: mln_string_view, capacity: Int) throws -> ([UInt8]?, Int) {
    var bytes = [UInt8](repeating: 0, count: capacity)
    var found = false
    let size = try bytes.withUnsafeMutableBufferPointer { buffer in
      try NativeMemory.withTemporary(0) { outSize in
  ...
  static func imageSourceCoordinates(_ map: OpaquePointer, sourceId: mln_string_view) throws -> [NativeLatLng]? {
  ...
  static func layerExists(_ map: OpaquePointer, layerId: mln_string_view) throws -> Bool {
    try NativeMemory.withTemporary(false) { exists in
      try checkStatus(mln_map_style_layer_exists(map, layerId, exists))
    }.value
  }

  static func layerType(_ map: OpaquePointer, layerId: mln_string_view) throws -> String? {
  ...
  static func layerJSON(_ map: OpaquePointer, layerId: mln_string_view) throws -> NativeJSONValue? {
  ...
  static func lightProperty(_ map: OpaquePointer, propertyName: mln_string_view) throws -> NativeJSONValue? {
  ...
  static func layerProperty(_ map: OpaquePointer, layerId: mln_string_view, propertyName: mln_string_view) throws -> NativeJSONValue? {
    let snapshot = try NativeMemory.withTemporary(Optional<OpaquePointer>.none) { snapshot in
  ...
  static func layerFilter(_ map: OpaquePointer, layerId: mln_string_view) throws -> NativeJSONValue? {
    let snapshot = try NativeMemory.withTemporary(Optional<OpaquePointer>.none) { snapshot in
  ...
  private static func copyStyleIdList(_ list: OpaquePointer) throws -> [String] {
