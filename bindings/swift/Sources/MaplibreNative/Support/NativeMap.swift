...
enum NativeMap {
  static func create(
    ...
    try NativeHandleFactory.create(nullDiagnostic: "mln_map_create returned a null map") { map in
  ...
  static func viewportOptions(_ map: OpaquePointer) throws -> mln_map_viewport_options {
  ...
  static func cameraForLatLngBounds(_ map: OpaquePointer, bounds: NativeLatLngBounds, fitOptions: UnsafePointer<mln_camera_fit_options>?) throws -> mln_camera_options {
    var camera = mln_camera_options_default()
    try checkStatus(mln_map_camera_for_lat_lng_bounds(map, bounds.native, fitOptions, &camera))
    return camera
  }

  static func cameraForLatLngs(_ map: OpaquePointer, coordinates: UnsafePointer<mln_lat_lng>?, count: Int, fitOptions: UnsafePointer<mln_camera_fit_options>?) throws -> mln_camera_options {
    var camera = mln_camera_options_default()
    try checkStatus(mln_map_camera_for_lat_lngs(map, coordinates, count, fitOptions, &camera))
    return camera
  }

  static func cameraForGeometry(_ map: OpaquePointer, geometry: UnsafePointer<mln_geometry>, fitOptions: UnsafePointer<mln_camera_fit_options>?) throws -> mln_camera_options {
    var camera = mln_camera_options_default()
    try checkStatus(mln_map_camera_for_geometry(map, geometry, fitOptions, &camera))
    return camera
  }

  static func latLngBoundsForCamera(_ map: OpaquePointer, camera: UnsafePointer<mln_camera_options>, unwrapped: Bool) throws -> NativeLatLngBounds {
  ...
  static func freeCameraOptions(_ map: OpaquePointer) throws -> mln_free_camera_options {
    var options = mln_free_camera_options_default()
    try checkStatus(mln_map_get_free_camera_options(map, &options))
    return options
  }

  static func projectionMode(_ map: OpaquePointer) throws -> mln_projection_mode {
    var mode = mln_projection_mode_default()
    try checkStatus(mln_map_get_projection_mode(map, &mode))
    return mode
  }

  static func pixelForLatLng(_ map: OpaquePointer, coordinate: NativeLatLng) throws -> NativeScreenPoint {
  ...
  static func latLngForPixel(_ map: OpaquePointer, point: NativeScreenPoint) throws -> NativeLatLng {
  ...
  static func pixelsForLatLngs(_ map: OpaquePointer, coordinates: [NativeLatLng]) throws -> [NativeScreenPoint] {
  ...
  static func latLngsForPixels(_ map: OpaquePointer, points: [NativeScreenPoint]) throws -> [NativeLatLng] {
