...
@Test func renderedQueryGeometryMaterializesNativeShapes() throws {
  try RenderedQueryGeometry.point(ScreenPoint(x: 1, y: 2)).nativeGeometry.withNativeGeometry { geometry in
    #expect(geometry.pointee.type == MLN_RENDERED_QUERY_GEOMETRY_TYPE_POINT.rawValue)
    #expect(geometry.pointee.data.point.x == 1)
    #expect(geometry.pointee.data.point.y == 2)
  }

  try RenderedQueryGeometry.lineString([ScreenPoint(x: 1, y: 2), ScreenPoint(x: 3, y: 4)]).nativeGeometry
    .withNativeGeometry { geometry in
      #expect(geometry.pointee.type == MLN_RENDERED_QUERY_GEOMETRY_TYPE_LINE_STRING.rawValue)
...
@Test func queryOptionsMaterializeLayerIdsAndFilters() throws {
  ...
  try options.nativeOptions.withNativeOptions { native in
    #expect(native != nil)
    #expect(native!.pointee.fields == MLN_RENDERED_FEATURE_QUERY_OPTION_LAYER_IDS.rawValue)
    #expect(native!.pointee.layer_id_count == 2)
    let firstLayerId = try NativeString.copyUTF8(data: native!.pointee.layer_ids![0].data, size: native!.pointee.layer_ids![0].size)
    #expect(firstLayerId == "roads")
    #expect(native!.pointee.filter?.pointee.type == MLN_JSON_VALUE_TYPE_ARRAY.rawValue)
  }
}
...
@Test func sourceQueryOptionsMaterializeLayerIdsAndFilters() throws {
  ...
  try options.nativeOptions.withNativeOptions { native in
    #expect(native != nil)
    #expect(native!.pointee.fields == MLN_SOURCE_FEATURE_QUERY_OPTION_SOURCE_LAYER_IDS.rawValue)
    #expect(native!.pointee.source_layer_id_count == 1)
    let firstLayerId = try NativeString.copyUTF8(data: native!.pointee.source_layer_ids![0].data, size: native!.pointee.source_layer_ids![0].size)
...
@Test func queriedFeatureCopiesNestedFeatureStateAndSourceMetadata() throws {
  ...
  var state = arena.nativeValue(.object([NativeJSONMember(key: "selected", value: .bool(true))]))
    ...
    try withUnsafePointer(to: &property) { propertyPointer in
      try withUnsafePointer(to: &state) { statePointer in
        ...
        let raw = mln_queried_feature(
          ...
          fields: MLN_QUERIED_FEATURE_SOURCE_ID.rawValue | MLN_QUERIED_FEATURE_SOURCE_LAYER_ID.rawValue | MLN_QUERIED_FEATURE_STATE.rawValue,
          feature: feature,
  ...
  #expect(copied.feature.geometry == .point(NativeLatLng(latitude: 1, longitude: 2)))
  #expect(copied.feature.properties == [NativeJSONMember(key: "name", value: .string("feature"))])
  #expect(copied.state == .object([NativeJSONMember(key: "selected", value: .bool(true))]))
}
