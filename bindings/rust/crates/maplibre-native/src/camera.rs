...
mod tests {
    ...
    fn camera_options_materialize_field_mask_and_values() {
        let options = CameraOptions::new()
            ...
            .with_anchor(ScreenPoint::new(5.0, 6.0));
        ...
        assert_eq!(raw.latitude, 10.0);
        assert_eq!(raw.longitude, 20.0);
        assert_eq!(raw.zoom, 3.5);
        assert_eq!(raw.padding.left, 2.0);
        assert_eq!(raw.anchor.y, 6.0);
    }

    #[test]
    fn camera_options_copy_from_native_uses_field_mask() {
        let mut raw = CameraOptions::new()
            .with_center(LatLng::new(1.0, 2.0))
            .with_zoom(4.0)
            .to_native();
        raw.bearing = 90.0;

        let copied = CameraOptions::from_native(raw);
    ...
    #[test]
    fn other_camera_descriptors_materialize_presence_masks() {
        let animation = AnimationOptions::new()
            .with_duration_ms(250.0)
            .with_velocity(1.0)
            .with_min_zoom(2.0)
        ...
        let fit = CameraFitOptions::new()
            .with_padding(EdgeInsets::new(1.0, 1.0, 1.0, 1.0))
            .with_bearing(45.0)
            .with_pitch(30.0)
            .to_native();
        assert_eq!(
            fit.fields & sys::MLN_CAMERA_FIT_OPTION_PADDING,
            sys::MLN_CAMERA_FIT_OPTION_PADDING
        );
        assert_eq!(
            fit.fields & sys::MLN_CAMERA_FIT_OPTION_BEARING,
            sys::MLN_CAMERA_FIT_OPTION_BEARING
        );
        assert_eq!(
            fit.fields & sys::MLN_CAMERA_FIT_OPTION_PITCH,
        ...
        let free = FreeCameraOptions::new()
            ...
            .with_orientation(Quaternion::new(0.0, 0.0, 0.0, 1.0))
            .to_native();
        assert_eq!(
            free.fields & sys::MLN_FREE_CAMERA_OPTION_POSITION,
