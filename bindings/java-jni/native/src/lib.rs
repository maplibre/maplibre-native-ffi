...
//!
//! This crate owns generated JNI exports and delegates shared ABI adaptation to
//! the Rust binding crates.
...
use java_bindgen::j2r::IntoRustType;
use java_bindgen::prelude::{JResult, java_bindgen};
use jni::objects::{
...
include!(concat!(env!("OUT_DIR"), "/bindgen_exports.rs"));

#[unsafe(no_mangle)]
pub extern "system" fn JNI_OnLoad(vm: JavaVM, _reserved: *mut c_void) -> jint {
    match catch_unwind(AssertUnwindSafe(|| register_generated_natives(&vm))) {
...
extern "system" fn log_set_callback<'local>(
    ...
    catch_unwind(AssertUnwindSafe(|| {
        log_set_callback_impl(&mut env, callback)
    }))
    .unwrap_or(sys::MLN_STATUS_NATIVE_ERROR)
}

fn log_set_callback_impl<'local>(env: &mut JNIEnv<'local>, callback: JObject<'local>) -> jint {
    if callback.is_null() {
        return jni_invalid_argument("JNI invalid argument");
    }
    let vm = match env.get_java_vm() {
    ...
    let result = unsafe { sys::mln_log_set_callback(Some(log_callback), state_id as *mut c_void) };
    match LOG_CALLBACK_STATE.lock() {
        Ok(mut registry) => {
            if result == sys::MLN_STATUS_OK {
...
extern "system" fn log_clear_callback(_env: JNIEnv<'_>, _class: JClass<'_>) -> jint {
    catch_unwind(log_clear_callback_impl).unwrap_or(sys::MLN_STATUS_NATIVE_ERROR)
}

fn log_clear_callback_impl() -> jint {
