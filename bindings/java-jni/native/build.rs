...
fn render_exports(entries: &[(String, NativeDeclaration)]) -> String {
    let mut out = String::from(
        r#"// Generated by bindings/java-jni/native/build.rs.

macro_rules! jbg_export {
    (
        fn $wrapper:ident,
        class $class:literal,
        ...
        with($env_for_call:ident, $class_arg:ident),
        call $call:block
    ) => {
        #[java_bindgen(
        ...
        fn $wrapper<'local>(
            ...
            let $class_arg = JClass::from(JObject::null());
            $(let _ = &$arg;)*
            $call
        }
    };
...
fn jni_symbol_name(class_name: &str, method_name: &str) -> String {
    format!(
        "Java_org_maplibre_nativejni_internal_bridge_{}_{}",
        escape_jni_identifier(class_name),
        escape_jni_identifier(method_name)
    )
}
