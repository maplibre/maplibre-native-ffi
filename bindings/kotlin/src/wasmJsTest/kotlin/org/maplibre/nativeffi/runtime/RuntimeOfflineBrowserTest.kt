 ...
 *
 * Every operation here is started on the owner thread and completes through the event queue, so an
 * operation is two calls with a pump between them. The database is in memory, which is what a
 * browser has: the module's default file system does not survive the page.
 */
class RuntimeOfflineBrowserTest {
  ...
  @Test
  fun anOfflineRegionIsCreatedObservedAndDeletedThroughCopiedResults(): Promise<JsAny?> =
    browserTest {
      maplibreScope {
  ...
  @Test
  fun aTakeThatFailsBeforeOwnershipMovesLeavesTheOperationRetryable(): Promise<JsAny?> =
    browserTest {
      maplibreScope {
        withRuntime(RuntimeOptions().apply { cachePath = ":memory:" }) { runtime ->
  ...
  @Test
  fun aTakeOfTheWrongResultKindIsRefusedBeforeCrossingIntoTheModule(): Promise<JsAny?> =
    browserTest {
      maplibreScope {
        withRuntime(RuntimeOptions().apply { cachePath = ":memory:" }) { runtime ->
  ...
  @Test
  fun aReleaseNativeRefusesLeavesTheHandleLiveForALaterOne(): Promise<JsAny?> = browserTest {
    maplibreScope {
      withRuntime(RuntimeOptions().apply { cachePath = ":memory:" }) { runtime ->
