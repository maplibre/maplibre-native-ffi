...
class DescriptorValidationBrowserTest {
  ...
  @Test
  fun signedCarriersRefuseValuesTheirUnsignedFieldsCannotHold() {
  ...
  @Test
  fun anEnumSentinelTheCApiCannotBeGivenIsRefusedBeforeDispatch() {
  ...
  @Test
  fun invalidMapAndValueInputsCarryNativesOwnRefusal() {
    withMap { _, map ->
      ...
      // This one is a process-global entry point rather than one belonging to a map, and it says
      // the same thing.
      val meters =
        assertFailsWith<InvalidArgumentException> {
  ...
  @Test
  fun aBackendThisModuleWasNotBuiltWithIsRefusedBeforeASessionExists() {
    withMap { _, map ->
