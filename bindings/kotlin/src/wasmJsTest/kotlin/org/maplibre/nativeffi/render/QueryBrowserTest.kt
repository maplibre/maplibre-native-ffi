...
class QueryBrowserTest {
  ...
  @Test
  fun aQueriedFeatureCarriesACopiedGeometryTreeAndItsIdentifiers() {
    withMap(WIDTH, HEIGHT) { runtime, map ->
      withSession(map) { session ->
        ...
        assertEquals(JsonValue.Bool(true), state.members.firstOrNull { it.key == "hovered" }?.value)
        ...
        repeat(ATTEMPTS) {
          if (
            cleared == JsonValue.Null ||
              (cleared is JsonValue.ObjectValue && cleared.members.isEmpty())
          ) {
            return@repeat
          }
          session.renderUpdate()
  ...
  @Test
  fun aClusterFeatureResolvesItsUnsignedIdAndBoundsItsLeaves() {
    withMap(WIDTH, HEIGHT) { runtime, map ->
      withSession(map) { session ->
   ...
   * via a block the binding allocates first, and the module's allocator can refuse it. What matters
   * is not the error — a caller sees an allocation failure either way — but whether the result
   * handle native had already produced went with it. A leaked one is silent, so it is replayed
   * against native, which is the only party that can say whether it is still there.
   */
  // Spec coverage: BND-066.
  @Test
  fun aFailedResultCopyDestroysTheNativeResultRatherThanLeakingIt() {
    withMap(WIDTH, HEIGHT) { runtime, map ->
      withSession(map) { session ->
        map.setStyleJson(POINTS_STYLE_JSON)
        map.jumpTo(
