...
class RuntimeEventBrowserTest {
  ...
  @Test
  fun aStyleLoadReportsItsOwnMapAndKeepsItsMessageAcrossTheNextPoll() {
    withMap { runtime, map ->
  ...
  @Test
  fun twoMapsAreEachNamedByTheirOwnStyleLoadEvent() {
    withRuntime { runtime ->
  ...
  @Test
  fun anEventFromAFutureDomainKeepsItsRawValuesAndCopiedPayload() {
    withRuntime { runtime ->
  ...
  @Test
  fun aKnownPayloadShorterThanItsStructIsReadAsUnknown() {
    withRuntime { runtime ->
      val full =
  ...
  @Test
  fun aMapEventWhoseMapHasBeenClosedNamesNoMap() {
    withRuntime { runtime ->
