...
private class VerificationResult(
  ...
  @Suppress("unused") private val code: Int = status.value
}
...
internal object CertificateVerifier {
  ...
  @get:Synchronized
  private var systemCertificateDirectory: File? =
    System.getenv("ANDROID_ROOT")?.let { rootPath -> File("$rootPath/etc/security/cacerts") }

  @get:Synchronized
  private val systemKeystore: KeyStore? =
    try {
  ...
  private fun verifyCertificateChain(
    ...
    // Will never throw `ArrayIndexOutOfBoundsException` because `rustls`'s `ServerCertVerifier`
    // trait
    ...
    //
    // Returns the "the properly ordered chain used for verification as a list of
    // X509Certificates.",
        ...
        if (BuildConfig.TEST) {
          ...
          return when (rootCause) {
            ...
            is CertificateNotYetValidException ->
              VerificationResult(StatusCode.Expired, rootCause.toString())
    ...
    // TEST ONLY: Mock test suite cannot attempt to check revocation status if no OSCP data has been
    // stapled,
    // because Android requires certificates to an specify OCSP responder for network fetch in this
    // case.
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
      ...
      // attempt a fetch for that certificate. Otherwise, it will attempt to fetch it from the
      // network.
      // Ref:
      // https://cs.android.com/android/platform/superproject/+/master:libcore/ojluni/src/main/java/sun/security/provider/certpath/RevocationChecker.java;l=694
      ...
      // "Note that when a `PKIXRevocationChecker` is added to `PKIXParameters`, it clones the
      ...
      try {
        ...
        // LetsEncrypt no longer include OCSP information (as OCSP is being deprecated) which
        // Android is not
        // happy with since it *only* tries OCSP by default. We aren't 100% decided on how to fix
        // this yet for real
        // (see https://github.com/rustls/rustls-platform-verifier/pull/179) so for now we implement
        // an out for
  ...
  //
  // Ref:
  // https://source.chromium.org/chromium/chromium/src/+/main:net/android/java/src/org/chromium/net/X509Util.java;l=339
  ...
  // Known means installed in root CA store, either a preset public CA or a custom one installed by
  // an enterprise/user.
  //
  // Ref:
  // https://source.chromium.org/chromium/chromium/src/+/main:net/android/java/src/org/chromium/net/X509Util.java;l=351
      ...
      systemCertificateDirectory?.let { loadedSystemCertificateDirectory ->
        ...
        while (true) {
          ...
          if (anchor == null) {
            ...
            // SAFETY: This logs a unique identifier (hash value) only in cases where a file within
            // the
            // system's root trust store is not a valid X509 certificate (extremely unlikely error).
            // The hash doesn't tell us any sensitive information about the invalid cert or reveal
            // any of
            // its contents - it just lets us ID the bad file if a user is having TLS failure
            // issues.
            ...
            if (
              (root.subjectX500Principal == anchor.subjectX500Principal) &&
                (root.publicKey == anchor.publicKey)
            ) {
