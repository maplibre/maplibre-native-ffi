...
class RuntimeEventsTest {
  ...
  fun oneDrainAfterStyleLoadReturnsEveryQueuedEvent(): Unit = runSuspendTest {
    withMap { runtime, map ->
      ...
      assertTrue(events.all { it.mapSource == map })
    }
  }
  ...
  @Test
  fun unknownPayloadTypeKeepsItsRawValueAndCopiesTheWholeWindow(): Unit = runSuspendTest {
    ...
    withMap { runtime, map ->
      ...
      runOnBackgroundThread {
        try {
          ...
          committed = map.setEventMask(RuntimeEventMask.ALL - RuntimeEventMask.MAP_TILE_ACTION)
