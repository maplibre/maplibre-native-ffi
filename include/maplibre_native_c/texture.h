...
typedef struct mln_webgpu_borrowed_texture_descriptor {
   ...
   *
   * This dimension is stated independently of extent so caller-owned texture
   * sizes that cannot be represented by logical extent times scale remain
   * expressible.
   */
  uint32_t physical_width;
 ...
 * The session creates a WebGPU texture on descriptor->context.device. Host
 ...
 * any use outside this session. On success, *out_session receives a handle the
 * caller destroys with mln_render_session_destroy().
 *
 ...
 *   owner thread.
 * - MLN_STATUS_UNSUPPORTED when session is not a texture session, the texture
 *   session uses a caller-owned target, or the configured backend does not
 *   support CPU readback.
