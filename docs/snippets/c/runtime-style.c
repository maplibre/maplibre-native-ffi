...
void add_cities_layer(mln_map map, const char* geojson_url) {
  ...
  mln_map_add_geojson_source_url(map, view("cities"), view(geojson_url), NULL);

  const char layer[] =
    "{\"id\":\"cities\",\"type\":\"circle\",\"source\":\"cities\"}";

  // An empty before-layer ID puts the layer on top.
  mln_map_add_style_layer_json(map, view(layer), view(""));

  mln_map_set_layer_property(
    map, view("cities"), view("circle-radius"), view("6.0")
  );
}

void refresh_cities(mln_map map, const char* new_url) {
  mln_map_set_geojson_source_url(map, view("cities"), view(new_url));
}
