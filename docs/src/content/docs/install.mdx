...
`macos-arm64-metal` or `linux-gnu-x64-vulkan`. OpenGL presets name their
context provider in place of the backend: `egl`, `wgl`, or `webgl`.
[Platforms](#platforms) gives the provider for each platform. Linux names its
C library explicitly with the `linux-gnu` and `linux-musl` platforms.
...
Linux prefixes include the shared library, a complete static archive, and its
companion platform archive. The archives can be linked into a static musl
executable, but EGL and Vulkan still load system graphics drivers at run time.
A fully static executable can therefore use non-rendering APIs, but rendering
needs a dynamic loader and compatible driver libraries.
...
iOS runs the Metal backend, on the device, in the simulator, and under Mac
Catalyst. The Mac Catalyst library is the iOS surface built against the macOS
SDK, which a Catalyst app links in place of the macOS library.
