...
Depend on the facade and one runtime payload. The facade owns every public
name; the payload carries one compiled artifact for one target and render
backend.

```bash frame="terminal"
pnpm add @maplibre/native-ffi @maplibre/native-ffi-runtime-linux-x64-vulkan
```

```ts
import { Maplibre } from "@maplibre/native-ffi";

const maplibre = await Maplibre.load();
const runtime = maplibre.createRuntime();
const map = runtime.createMap({ width: 512, height: 512 });
```

Both module formats come from one build, so `require()` reaches the same API.
Installing more than one compatible payload makes `load()` ask which one to
use, because the binding picks no runtime for you.

A browser loads a WebAssembly payload instead, through its own entry point, so
a server application never pulls WebAssembly loading in:

```bash frame="terminal"
pnpm add @maplibre/native-ffi @maplibre/native-ffi-runtime-emscripten-wasm32-opengl
```
...
The payload is threaded, so a page must be [cross-origin isolated][coi] or the
browser withholds the shared memory it needs. Serve the page with:
...
A bundled application usually serves the payload's `.mjs` and `.wasm` itself
rather than letting the loader resolve the package. Name them with
`loadBrowser({ moduleUrl })`; the payload exports both files as subpaths so a
build can copy them out.
