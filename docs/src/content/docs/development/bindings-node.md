...
Attach methods return `RenderSessionHandle`. Texture readback supports caller
provided `Uint8Array` or `ArrayBuffer` storage and may offer a convenience
...
non-`Promise` return, and the runtime treats returned promises as unsupported.
