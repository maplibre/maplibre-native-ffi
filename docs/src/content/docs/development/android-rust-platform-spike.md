...
the first build may stub features that would otherwise require curl, image codec
libraries, or full ICU.
...
- Add a Rust static library target only after the C++ Android platform source
  list configures and links with stubs.
...
| HTTP                         | Async request, cancellation, headers, ranges, cache metadata, and error mapping | Stub or require C API resource provider first                   | `ureq` + `rustls`, or `reqwest` + `rustls` |
| PNG decode                   | Decode encoded bytes to premultiplied RGBA8                                     | Rust replacement after image stub                               | `png` or `image`                           |
| JPEG decode                  | Decode encoded bytes to RGBA8                                                   | Rust replacement after image stub                               | `zune-jpeg`, `jpeg-decoder`, or `image`    |
| WebP decode                  | Decode encoded bytes to RGBA8                                                   | Rust replacement after image stub                               | `image-webp` or `image`                    |
...
2. `maplibre_native_c` links with Android platform source selection and stubs.
3. A minimal native smoke path calls a basic exported C API function.
4. Rust static library links into `maplibre_native_c`.
5. Rust PNG/JPEG/WebP decoding replaces the Android image decode stub.
6. HTTP is either handled by the existing C API resource provider or replaced by
   a Rust HTTP implementation behind the same MapLibre `FileSource` behavior.
