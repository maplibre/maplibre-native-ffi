...
request handle and mirroring the C API's set-cancel-callback operation. The
binding invokes the host callback on the thread that cancels the request, which
is the owner thread inside a map or runtime call and a MapLibre thread
otherwise. The binding converts host failures inside the callback to documented
behavior, and lets the callback complete or release the same request. A binding
that blocks native threads on a host lock, such as an interpreter lock, releases
that lock around the map and runtime calls that can cancel requests. Registering
after cancellation runs the callback before the registration returns. Release
waits for a cancel callback running on another thread and returns without
waiting from inside that callback. A released request rejects registration as
closed.
...
| BND-198 | A cancel callback registered on a handled request runs once when MapLibre discards the request, may release that request from inside the callback, and is not invoked for a request the provider completed. |
