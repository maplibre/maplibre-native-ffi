// The process-global MapLibre log callback and the helper that reports a
// failed C API call with its thread-local diagnostic.
...
/// Reports a failed C API call. Call on the thread that made it, before that
/// thread's next C API call replaces the thread-local diagnostic.
