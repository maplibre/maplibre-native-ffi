...
static void render_session_maintenance_rejects_null_raw_handles(void) {
  TEST_ASSERT_EQUAL_INT(
    ...
    mln_render_session_reduce_memory_use(MLN_HANDLE_NULL)
  );
  TEST_ASSERT_EQUAL_INT(
    MLN_STATUS_INVALID_ARGUMENT, mln_render_session_clear_data(MLN_HANDLE_NULL)
  );
  TEST_ASSERT_EQUAL_INT(
    MLN_STATUS_INVALID_ARGUMENT,
    mln_render_session_dump_debug_logs(MLN_HANDLE_NULL)
  );
}
...
static void render_session_rejects_stale_raw_handles(void) {
  ...
  TEST_ASSERT_EQUAL_INT(
    MLN_STATUS_INVALID_ARGUMENT,
    mln_render_session_render_update(stale_session, &result, &needs_repaint)
  );
  TEST_ASSERT_EQUAL_INT(
