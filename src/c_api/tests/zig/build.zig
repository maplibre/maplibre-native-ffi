...
fn addCTests(b: *std.Build, options: BuildOptions) *std.Build.Step.Compile {
    ...
    if (options.render_backend == .opengl and options.target.result.os.tag == .linux) {
        // egl_support_linux.c creates a real EGL pbuffer for lifecycle tests.
        // It uses dlopen/dlsym so the test binary avoids linking -lEGL directly.
