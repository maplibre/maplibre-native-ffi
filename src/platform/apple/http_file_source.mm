...
static NSNumber* MLNEffectivePort(NSURL* url) {
  if (url.port) return url.port;
  if ([url.scheme caseInsensitiveCompare:@"https"] == NSOrderedSame)
    return @443;
  if ([url.scheme caseInsensitiveCompare:@"http"] == NSOrderedSame) return @80;
  return nil;
}

static BOOL MLNSameOrigin(NSURL* first, NSURL* second) {
  return first && second && first.scheme && second.scheme && first.host &&
         second.host &&
         ...
         [MLNEffectivePort(first) isEqual:MLNEffectivePort(second)];
}
...
- (void)registerTask:(NSURLSessionTask*)task
  ...
  objc_setAssociatedObject(
    task, &MLNTransformedHeaderNamesKey, [headers copy],
...
class HTTPRequest : public AsyncRequest {
 ...
 private:
  ...
  util::AsyncTask async{[this] {
    // Calling `callback` may result in deleting `this`. Copy data to
    // temporaries first.
...
class HTTPFileSource::Impl {
 public:
  Impl(
    const ResourceOptions& resourceOptions_, const ClientOptions& clientOptions_
  )
      : resourceOptions(resourceOptions_.clone()),
        clientOptions(clientOptions_.clone()) {
    NSURLSessionConfiguration* sessionConfig =
      MLNNativeNetworkManager.sharedManager.sessionConfiguration;
    redirectDelegate = [[MLNHTTPRedirectDelegate alloc] init];
  ...
  MLNHTTPRedirectDelegate* redirectDelegate = nil;
...
NSString* HTTPFileSource::Impl::getUserAgent() const {
  ...
  if (sdkBundle) {
    NSString* versionString =
      sdkBundle.infoDictionary[@"CFBundleShortVersionString"];
    if (versionString) {
      [userAgentComponents
        addObject:[NSString
                    stringWithFormat:@"%@/%@",
                                     sdkBundle.infoDictionary[@"CFBundleName"],
                                     versionString]];
    }
  }

  // Avoid %s here because it inserts hidden bidirectional markers on macOS when
  ...
  if (
    [NSProcessInfo instancesRespondToSelector:@selector(operatingSystemVersion)]
  ) {
  ...
  if (systemVersion) {
    [userAgentComponents
      addObject:[NSString
                  stringWithFormat:@"%@/%@", systemName, systemVersion]];
  }
...
NSBundle* HTTPFileSource::Impl::getSDKBundle() const {
  ...
  if (
    bundle &&
    ![bundle.infoDictionary[@"CFBundlePackageType"] isEqualToString:@"FMWK"]
  ) {
    // For static frameworks, the class is contained in the application bundle
    // rather than the framework bundle.
    bundle = [NSBundle
      bundleWithPath:[bundle.privateFrameworksPath
...
BOOL isValidMapboxEndpoint(NSURL* url) {
  return (
    [url.host isEqualToString:@"mapbox.com"] ||
...
NSURL* resourceURL(const Resource& resource) {
  ...
  if (isValidMapboxEndpoint(url)) {
    ...
    if (resource.usage == Resource::Usage::Offline) {
      [queryItems addObject:[NSURLQueryItem queryItemWithName:@"offline"
                                                        value:@"true"]];
    }
...
static std::uint32_t resourceKindToAbi(Resource::Kind kind) {
  switch (kind) {
...
std::unique_ptr<AsyncRequest> HTTPFileSource::request(
  const Resource& resource, std::function<void(Response)> callback
) {
  auto request = std::make_unique<HTTPRequest>(std::move(callback));
  auto shared = request->shared;  // Explicit copy so that it also gets copied
                                  // into the completion handler block below.

  @autoreleasepool {
    NSURL* url = resourceURL(resource);
    [MLNNativeNetworkManager.sharedManager
      debugLog:@"Requesting URI: %@", url.relativePath];
    ...
    if (resource.dataRange) {
      NSString* rangeHeader = [NSString
        stringWithFormat:@"bytes=%lld-%lld",
    ...
    NSMutableArray<NSString*>* transformedHeaderNames = [NSMutableArray array];
    const BOOL isHTTP =
      [url.scheme caseInsensitiveCompare:@"http"] == NSOrderedSame ||
      [url.scheme caseInsensitiveCompare:@"https"] == NSOrderedSame;
    auto transformedHeaders =
      isHTTP ? mln::core::invoke_http_header_transform(
                 impl->getResourceOptions().platformContext(),
                 resourceKindToAbi(resource.kind), url.absoluteString.UTF8String
               )
             : mln::core::HttpHeaders{};
    for (const auto& [name, value] : transformedHeaders) {
      NSString* field = @(name.c_str());
      [req setValue:@(value.c_str()) forHTTPHeaderField:field];
    ...
    __block BOOL invalidateSessionOnCompletion = NO;

    // Use the delegate's session if there is one, otherwise use the one that
    // was created when this class was constructed.
    MLNNativeNetworkManager* networkManager =
      MLNNativeNetworkManager.sharedManager;
    if (
      [networkManager.delegate
        respondsToSelector:@selector(sessionForNetworkManager:)]
    ) {
      session =
        [networkManager.delegate sessionForNetworkManager:networkManager];
    }
    ...
    request->task = [session
      dataTaskWithRequest:req
        completionHandler:^(NSData* data, NSURLResponse* res, NSError* error) {
          if (invalidateSessionOnCompletion) {
          ...
          if (
            [networkManager.delegate
              respondsToSelector:@selector(didReceiveResponse:)]
          ) {
            MLNInternalNetworkResponse* networkResponse =
              [MLNInternalNetworkResponse responseWithData:data
                                               urlResponse:res
                                                     error:error];
            networkResponse =
              [networkManager.delegate didReceiveResponse:networkResponse];
          ...
          if (error && [error code] == NSURLErrorCancelled) {
            [MLNNativeNetworkManager.sharedManager
              cancelDownloadEventForResponse:res];
            return;
          }
          [MLNNativeNetworkManager.sharedManager
            stopDownloadEventForResponse:res];
          Response response;
          ...
          if (error) {
            ...
            [MLNNativeNetworkManager.sharedManager
              debugLog:@"Requesting %@ returned responseCode: %lu",
                       res.URL.relativePath, responseCode];

            NSDictionary* headers = [(NSHTTPURLResponse*)res allHeaderFields];
            NSString* cache_control = [headers objectForKey:@"Cache-Control"];
            if (cache_control) {
              const auto cc =
                http::CacheControl::parse([cache_control UTF8String]);
            ...
            if (last_modified) {
              response.modified =
                util::parseTimestamp([last_modified UTF8String]);
            }
            ...
            response.error = std::make_unique<Error>(
              Error::Reason::Other, "Response class is not NSHTTPURLResponse"
            );
          }
