...
constexpr std::size_t kCallbackTokens =
  kMaxActiveRequests * kCallbackGenerations;
...
std::optional<std::string> headerValue(
  Http_Headers* headers, const char* name
) {
  if (!headers) {
    return std::nullopt;
  }
...
Response makeNetworkError(const Resource& resource, uint32_t errCode) {
  ...
  response.error = std::make_unique<Response::Error>(
    errorReason(errCode), errorMessage(resource, errCode)
  );
  return response;
}
...
Response makeResponse(
  const Resource& resource, Http_Response* httpResponse, uint32_t errCode
) {
  ...
  if (!httpResponse) {
    response.error = std::make_unique<Response::Error>(
      Response::Error::Reason::Other, "OpenHarmony HTTP returned no response"
    );
    return response;
  }
  ...
  if (code == 200 || code == 206) {
    if (httpResponse->body.buffer && httpResponse->body.length > 0) {
      response.data = std::make_shared<std::string>(
        httpResponse->body.buffer, httpResponse->body.length
      );
    } else {
      response.data = std::make_shared<std::string>();
    }
    ...
    response.error = std::make_unique<Response::Error>(
      ...
      http::parseRetryHeaders(
        headerValue(httpResponse->headers, "retry-after"),
...
class SlotPool {
 public:
  ...
  static void handleResponse(
    std::size_t token, Http_Response* response, uint32_t errCode
  );
 ...
 private:
  ...
  static std::optional<std::size_t> assignSlotLocked(
...
constexpr auto makeResponseCallbacks(std::index_sequence<Tokens...>) {
  return std::array<Http_ResponseCallback, sizeof...(Tokens)>{
    &responseThunk<Tokens>...
  };
}

template <std::size_t... Tokens>
constexpr auto makeCanceledCallbacks(std::index_sequence<Tokens...>) {
  return std::array<Http_OnVoidCallback, sizeof...(Tokens)>{
    &canceledThunk<Tokens>...
  };
}

constexpr auto responseCallbacks =
  makeResponseCallbacks(std::make_index_sequence<kCallbackTokens>{});
constexpr auto canceledCallbacks =
  makeCanceledCallbacks(std::make_index_sequence<kCallbackTokens>{});
...
class RequestState : public std::enable_shared_from_this<RequestState> {
 public:
  ...
  void startNativeRequest() {
    ...
    if (
      !setHeader(newHeaders, "Accept-Encoding", "gzip, deflate") ||
    ...
    if (resource.dataRange) {
      const auto range = std::string{"bytes="} +
                         util::toString(resource.dataRange->first) + "-" +
    ...
    if (resource.priorEtag) {
      if (!setHeader(
            newHeaders, "If-None-Match", resource.priorEtag->c_str()
          )) {
        OH_Http_DestroyHeaders(&newHeaders);
        OH_Http_Destroy(&newRequest);
        complete(nullptr, OH_HTTP_OUT_OF_MEMORY);
    ...
    const int result = OH_Http_Request(
      newRequest, SlotPool::responseCallback(*activeToken), handler
    );
    {
      std::scoped_lock lock(mutex);
      starting = false;
 ...
 private:
  static bool setHeader(
    Http_Headers* headers, const char* name, const char* value
  ) {
  ...
  void finish(std::optional<Response> response) {
    ...
    if (shouldCallback) {
      runLoop.invoke([state = shared_from_this(),
...
std::optional<std::size_t> SlotPool::assignSlotLocked(
  const std::shared_ptr<RequestState>& state
) {
  for (std::size_t offset = 0; offset < slots.size(); ++offset) {
    ...
    const auto token = index * kCallbackGenerations +
                       (slots[index].generation % kCallbackGenerations);
