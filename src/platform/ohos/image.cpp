...
void checkImageResult(Image_ErrorCode code, const char* operation) {
  if (code != IMAGE_SUCCESS) {
    throw std::runtime_error(
      std::string(operation) + " failed with " + imageErrorCodeMessage(code)
    );
  }
}
...
std::string pixelmapDescription(
  uint32_t width, uint32_t height, uint32_t rowStride, int32_t format,
  int32_t alphaType
) {
...
using ImageSourcePtr =
  std::unique_ptr<OH_ImageSourceNative, ImageSourceDeleter>;
using DecodingOptionsPtr =
  std::unique_ptr<OH_DecodingOptions, DecodingOptionsDeleter>;
using PixelmapPtr = std::unique_ptr<OH_PixelmapNative, PixelmapDeleter>;
using PixelmapInfoPtr =
  std::unique_ptr<OH_Pixelmap_ImageInfo, PixelmapInfoDeleter>;

std::size_t checkedByteCount(
  std::size_t width, std::size_t height, const char* description
) {
...
PremultipliedImage decodeImage(const std::string& string) {
  ...
  checkImageResult(
    OH_ImageSourceNative_CreateFromData(data.data(), data.size(), &sourceRaw),
  ...
  checkImageResult(
    OH_DecodingOptions_Create(&optionsRaw), "OH_DecodingOptions_Create"
  );
  DecodingOptionsPtr options(optionsRaw);
  checkImageResult(
    OH_DecodingOptions_SetPixelFormat(options.get(), PIXEL_FORMAT_RGBA_8888),
  ...
  checkImageResult(
    OH_ImageSourceNative_CreatePixelmap(
      source.get(), options.get(), &pixelmapRaw
    ),
    "OH_ImageSourceNative_CreatePixelmap"
  );
  ...
  checkImageResult(
    OH_PixelmapImageInfo_Create(&infoRaw), "OH_PixelmapImageInfo_Create"
  );
  PixelmapInfoPtr info(infoRaw);
  checkImageResult(
    OH_PixelmapNative_GetImageInfo(pixelmap.get(), info.get()),
  ...
  checkImageResult(
    OH_PixelmapImageInfo_GetWidth(info.get(), &width),
    "OH_PixelmapImageInfo_GetWidth"
  );
  checkImageResult(
    OH_PixelmapImageInfo_GetHeight(info.get(), &height),
    "OH_PixelmapImageInfo_GetHeight"
  );
  checkImageResult(
    OH_PixelmapImageInfo_GetRowStride(info.get(), &rowStride),
    "OH_PixelmapImageInfo_GetRowStride"
  );
  checkImageResult(
    OH_PixelmapImageInfo_GetPixelFormat(info.get(), &pixelFormat),
    "OH_PixelmapImageInfo_GetPixelFormat"
  );
  checkImageResult(
    OH_PixelmapImageInfo_GetAlphaType(info.get(), &alphaType),
    "OH_PixelmapImageInfo_GetAlphaType"
  );

  const auto description =
    pixelmapDescription(width, height, rowStride, pixelFormat, alphaType);
  if (width == 0 || height == 0) {
    throw std::runtime_error(
      "OHOS image decoder returned empty image dimensions: " + description
    );
  }
  if (
    ...
    throw std::runtime_error(
      "OHOS image decoder returned unsupported pixel format: " + description
    );
  }
